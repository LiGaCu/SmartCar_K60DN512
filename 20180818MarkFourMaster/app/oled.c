...
//OLED显示屏
//定义输出
#define LED_SCLK(x) LPLD_GPIO_Output_b(PTD, 9, x)// d0
#define LED_SDA(x)  LPLD_GPIO_Output_b(PTD, 8, x)//d1
#define LED_RST(x)  LPLD_GPIO_Output_b(PTD, 11, x)//复位低能电平
#define LED_DC(x)   LPLD_GPIO_Output_b(PTD, 10, x)//偏置常低
//OLED显示函数
...
void led_gpio_int(void)
{
  ...
  led_Port_init.GPIO_Pins = GPIO_Pin9|GPIO_Pin8|GPIO_Pin11|GPIO_Pin10;
